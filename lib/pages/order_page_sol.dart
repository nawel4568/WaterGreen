...
class OrderPageSol extends StatefulWidget {
  Humidity humidity;
  OrderPageSol({Key? key,required this.humidity}) : super(key: key);
...
class _OrderPageSolState extends State<OrderPageSol> with SingleTickerProviderStateMixin{
  ...
  void _connectToDevice(BluetoothDiscoveryResult device) async{
    try{
      ...
      _dataStreamSubscription = _connection.input!.listen((data) { // Update this line
        setState(() {
          _humidityData += utf8.decode(data);
          if (_humidityData.contains('\n')) {
            moistureValue = int.parse(_humidityData);
            // Do something with the moisture value
            print(moistureValue);
            // Reset receivedData
            _humidityData = "";
          }
